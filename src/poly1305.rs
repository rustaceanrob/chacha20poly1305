...
impl Poly1305 {
    ...
    pub(crate) fn add(&mut self, message: &[u8]) {
        ...
        while i < message.len() / 16 {
            ...
            // print!("Accumulator plus message: ");
            // print_acc(&self.acc);
            self.r_times_a();
        ...
        if message.len() % 16 > 0 {
            ...
            // print!("Last accumulator after message add: ");
            // print_acc(&self.acc);
            self.r_times_a();
            // print!("Accumulator times R mod P: ");
            // print_acc(&self.acc);
    ...
    fn r_times_a(&mut self) {
        // multiply and reduce
        let mut t = [0; 5];
        t[0] +=      self.r[0]  as u64 * self.acc[0] as u64;
        t[1] +=      self.r[0]  as u64 * self.acc[1] as u64;
        t[2] +=      self.r[0]  as u64 * self.acc[2] as u64;
        t[3] +=      self.r[0]  as u64 * self.acc[3] as u64;
        t[4] +=      self.r[0]  as u64 * self.acc[4] as u64;
        t[0] += (5 * self.r[1]) as u64 * self.acc[4] as u64;
        t[1] +=      self.r[1]  as u64 * self.acc[0] as u64;
        t[2] +=      self.r[1]  as u64 * self.acc[1] as u64;
        t[3] +=      self.r[1]  as u64 * self.acc[2] as u64;
        t[4] +=      self.r[1]  as u64 * self.acc[3] as u64;
        t[0] += (5 * self.r[2]) as u64 * self.acc[3] as u64;
        t[1] += (5 * self.r[2]) as u64 * self.acc[4] as u64;
        t[2] +=      self.r[2]  as u64 * self.acc[0] as u64;
        t[3] +=      self.r[2]  as u64 * self.acc[1] as u64;
        t[4] +=      self.r[2]  as u64 * self.acc[2] as u64;
        t[0] += (5 * self.r[3]) as u64 * self.acc[2] as u64;
        t[1] += (5 * self.r[3]) as u64 * self.acc[3] as u64;
        t[2] += (5 * self.r[3]) as u64 * self.acc[4] as u64;
        t[3] +=      self.r[3]  as u64 * self.acc[0] as u64;
        t[4] +=      self.r[3]  as u64 * self.acc[1] as u64;
        t[0] += (5 * self.r[4]) as u64 * self.acc[1] as u64;
        t[1] += (5 * self.r[4]) as u64 * self.acc[2] as u64;
        t[2] += (5 * self.r[4]) as u64 * self.acc[3] as u64;
        t[3] += (5 * self.r[4]) as u64 * self.acc[4] as u64;
        t[4] +=      self.r[4]  as u64 * self.acc[0] as u64;
        // carry
        for i in 0..4 {
            t[i + 1] += t[i] >> CARRY;
        }
