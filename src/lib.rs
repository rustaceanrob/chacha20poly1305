...
impl ChaCha20Poly1305 {
    ...
    pub fn encrypt<'a>(self, plaintext: &'a mut [u8], aad: Option<&'a [u8]>, buffer: &'a mut [u8]) -> &'a [u8] {
        ...
        for i in 0..aad_len.len() {
            len_buffer[i] = aad_len[i]
        }
...
impl Poly1305 {
    fn new(key: [u8; 32]) -> Self {
        let mut r: [u8; 16] = key[..16].try_into().expect("Key is at least 16 bytes long.");
        r[3] &= 15;
        r[7] &= 15;
        r[11] &= 15;
        r[15] &= 15;
    ...
    fn add(&mut self, message: &[u8]) {
        ...
        while i < message.len() / 16 {
            ...
            self.acc = (self.r.clone() * (self.acc.clone() + n)).modpow(&BigUint::from_slice(&[1]), &self.modulus);
            i += 1;
        }
        if message.len() % 16 > 0 {
    ...
    fn get_tag(self) -> [u8; 16] {
        ...
        tag.to_bytes_le()[..16].try_into().expect("Message tag should be at least 16 bytes.")
    }
}
